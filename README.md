...
- **Model registry + in-place quantization.** `replace_with_quantized(model, scheme)` walks any `nn.Module` tree and swaps `nn.Conv2d`/`nn.Linear` for their quantized cousins **without changing attribute paths**, preserving state-dict keys for HuggingFace checkpoint compatibility. 23 architectures registered: VGG3 (MNIST/FMNIST), VGG7 (CIFAR-10/100), ResNet18 (Imagenette, 64×64), ResNet{18,34,50}, MobileNetV2, ViT-B/16, each in ImageNet and CIFAR variants. VGG3/VGG7 and the Imagenette ResNet18 are BNN topologies (`Hardtanh` + `QuantizedActivation` at every activation site, so w1a1/W1A_n_ is expressible); the torchvision wrappers are ReLU-based and are weight-quantization only.
